...
public class StreamTest {
    ...
    public void distinctTest(){
        ...
        List<Long> collect = source.stream().map(UserCourseDto::getClassId).collect(Collectors.toList());
        source.stream().map(UserCourseDto::getClassId).distinct().forEach(classId->{
            System.out.println(classId);
        });
        source.stream().distinct().forEach(classId->{
            System.out.println(classId);
        });
        source.stream().forEach(dto->{
            if(dto.equals(new UserCourseDto(600165l,1006l,35l,27l,100001936l))){
                System.out.println(dto);
            }
        });
    }
